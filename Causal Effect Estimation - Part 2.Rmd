...
Second Stage:
$$
\text{TimeDriving_Minutes}_{it} = \beta_1 \cdot \text{(EverApplied}_{i} \cdot \text{After}_{it} \text{)}_{predicted} + \mu_{\text{city}_i, \text{expstart}_i, \text{date}_t} + \epsilon_{it}
$$

Recall that the $\mu_{\text{city}_i, \text{expstart}_i, \text{date}_t}$ (CohortXTime) FE is perfectly colinear with `After`. Hence, it is not included in the second stage.
